...
public class OsmiumCommands {
    ...
    @Command(names = {"openserverselector"}, playerOnly = true)
    public boolean open(Player sender) {
        cc.invictusgames.invictus.profile.Profile profile = InvictusBukkit.getInstance().getProfileService().getProfile(sender.getUniqueId());

        if (profile.isRequiresAuthentication()) {
            sender.sendMessage(CC.RED + "Please authenticate using " + CC.YELLOW + "/auth <code>" + CC.RED +".");
            return false;
        }

        new ServerSelectorMenu(plugin).openMenu(sender);
        return true;
    }
